...
/***********************************************************************
*
*  $TC Tipo de dados: Pto para strings
*
*
***********************************************************************/
...
/***********************************************************************
*
*  $FC Função: STU  -Preencher os campos de strings na ptoString
*
***********************************************************************/

   ptoString * prchStr (char * sIniciais, char * sNomeComp) {

	   ptoString * sElem;
	   sElem = ( ptoString *) malloc( sizeof (ptoString));

	   if (sElem == NULL) {
		   return NULL;
	   }

	   sElem->sIni = sIniciais;
	   sElem->sNome = sNomeComp;

	   return sElem;
   } /* Fim da função: STU -Preencher os campos de strings na ptoString */

/***********************************************************************
*
*  $FC Função: STU  -Inserir em ordem
*
***********************************************************************/

   void InsOrde (LIS_tppLista pLista, char * sIni, char * sNome) {
	   
	   ptoString * sElem;
	   IrInicioLista( pLista );

	   if (pLista->pElemCorr == NULL) {
		   sElem = prchStr (sIni, sNome);
		   pLista->pElemCorr = CriarElemento (pLista, sElem);
	   } /* if */
	   while (pLista->pElemCorr != NULL) {
		   int n;
		   ptoString* x =(ptoString*) pLista->pElemCorr->pValor;
		   n = strcmp(x->sIni,sIni);

		   if (n>0 || n==0) {
			   /* 1° elemento > que 2° */
				sElem = prchStr (sIni, sNome);
				LIS_InserirElementoAntes (pLista , sElem);
				return;
		   } /* 2° elemento > que 1° */
		   else if (pLista->pElemCorr->pProx == NULL) {
				sElem = prchStr (sIni, sNome);
				LIS_InserirElementoApos (pLista , sElem);
		   } /* else if */
		   LIS_AvancarElementoCorrente( pLista , pLista->numElem );
	   }

   }/* Fim da função: STU -Inserir em ordem */

/***********************************************************************
*
*  $FC Função: STU  -Printar iniciais e nomes de todos
*
***********************************************************************/

	void printLis (LIS_tppLista pLista) {
	   IrInicioLista( pLista );

		while (pLista->pElemCorr!=NULL) {
			printf("%s, %s\n", pLista->pElemCorr->pValor->sIni, pLista->pElemCorr->pValor->sNome);
			LIS_AvancarElementoCorrente( pLista, pLista->numElem);
		}
	}/* Fim da função: STU -Printar iniciais e nomes de todos */
